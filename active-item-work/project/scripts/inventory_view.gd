...
func _rebuild_card_pages() -> void:
	...
	for card_id in InventoryState.owned_cards:
		_add_card_view(card_grid, card_id)
	for card_id in InventoryState.deck_cards:
		_add_card_view(deck_grid, card_id)
	card_empty_label.visible = InventoryState.owned_cards.is_empty()


func _add_card_view(grid: GridContainer, card_id: int) -> void:
	var data := InventoryState.get_card(card_id)
	if data.is_empty():
		return
	var card := CARD_VIEW_SCENE.instantiate() as BattleCardView
	grid.add_child(card)
	card.configure(data, Vector2(104, 148))
