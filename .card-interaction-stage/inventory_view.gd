...
func _add_card_view(grid: GridContainer, card_instance: Dictionary) -> void:
	...
	card_cell.custom_minimum_size = Vector2(104, 148) * 1.2 + Vector2(4, 4)
	...
	card_cell.mouse_entered.connect(
		_on_browse_card_hover.bind(card, true)
	)
	card_cell.mouse_exited.connect(
		_on_browse_card_hover.bind(card, false)
	)
	card.card_clicked.connect(
		_on_browse_card_pressed.bind(card, data, card_instance)
	)


func _on_browse_card_hover(card: BattleCardView, active: bool) -> void:
...
func _on_browse_card_pressed(
	card: BattleCardView,
	data: Dictionary,
	card_instance: Dictionary
) -> void:
	card.set_hover_active(false)
	card_detail_overlay.show_card(data, card_instance)
