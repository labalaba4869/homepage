...
TABLES = (
    TableSpec(
        ...
        int_fields=("card_equality", "buy_price", "shop_enabled"),
...
def validate_card_tables(tables: dict[str, list[list[Any]]]) -> None:
    ...
    for card in cards:
        ...
        if int(card["buy_price"]) < 0:
            raise ValueError(
                f"cards/card_id {card_id} buy_price must be >= 0"
            )
        if int(card["shop_enabled"]) not in (0, 1):
            raise ValueError(
                f"cards/card_id {card_id} shop_enabled must be 0 or 1"
            )
