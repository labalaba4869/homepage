...
func get_save_data() -> Dictionary:
	...
	return {
		"player_hp": player_hp,
		"pending_battle_attack_bonus": pending_battle_attack_bonus,
		"pending_battle_energy_bonus": pending_battle_energy_bonus,
		"pending_instant_normal_kill": pending_instant_normal_kill,
		"move_speed_bonus_percent": _move_speed_bonus_percent,
		"move_speed_seconds_left": get_move_speed_seconds_left(),
	}
...
func apply_save_data(data: Dictionary) -> void:
	...
	pending_battle_attack_bonus = maxi(
		int(data.get("pending_battle_attack_bonus", 0)),
		0
	)
	pending_battle_energy_bonus = maxi(
		int(data.get("pending_battle_energy_bonus", 0)),
		0
	)
	pending_instant_normal_kill = bool(data.get("pending_instant_normal_kill", false))
	_move_speed_bonus_percent = maxi(int(data.get("move_speed_bonus_percent", 0)), 0)
	var speed_seconds := maxi(int(data.get("move_speed_seconds_left", 0)), 0)
	_move_speed_buff_ends_msec = (
		Time.get_ticks_msec() + speed_seconds * 1000 if speed_seconds > 0 else 0
	)
	set_player_hp(int(data.get("player_hp", player_max_hp)))
...
func try_instant_enemy_defeat(
	...
	defeated_enemy_ids[_enemy_state_key(enemy_id, enemy_instance_id)] = true
...
func begin_battle(
	...
	current_enemy_instance_id = enemy_instance_id
...
func finish_battle(result: String) -> void:
	...
	match result:
		"victory":
			defeated_enemy_ids[
				_enemy_state_key(current_enemy_id, current_enemy_instance_id)
			] = true
...
func is_enemy_defeated(enemy_id: int, enemy_instance_id := "") -> bool:
	return defeated_enemy_ids.has(
		_enemy_state_key(enemy_id, enemy_instance_id)
	)
...
func _enemy_state_key(enemy_id: int, enemy_instance_id: String) -> String:
	if not enemy_instance_id.is_empty():
		return enemy_instance_id
