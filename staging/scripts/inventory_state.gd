...
const MIN_DECK_CARD_COUNT := 10
const MAX_DECK_CARD_COUNT := 20

var items: Dictionary = {}
var item_qualities: Dictionary = {}
var loot_groups: Dictionary = {}
var cards: Dictionary = {}
var card_qualities: Dictionary = {}
...
func ensure_initialized() -> void:
	...
	card_qualities = GameDataScript.load_qualities()
	...
	for card_id in configured_cards:
		deck_cards.append(_create_card_instance(int(card_id)))
...
func get_save_data() -> Dictionary:
	...
	return {
		...
		"active_bindings": active_bindings.duplicate(true),
		"owned_cards": owned_cards.duplicate(true),
		"deck_cards": deck_cards.duplicate(true),
		"player_coins": player_coins,
		"next_card_instance_id": _next_card_instance_id,
		"pending_quality_upgrade_percent": pending_quality_upgrade_percent,
		"pending_card_chance_boost_percent": pending_card_chance_boost_percent,
		"pending_guaranteed_card": pending_guaranteed_card,
		"pending_force_filled_slots": pending_force_filled_slots,
	}


func apply_save_data(data: Dictionary) -> void:
	ensure_initialized()
	var restored_slots := _restore_inventory_slots(data.get("inventory_slots", []))
	inventory_slots = restored_slots
	var restored_bindings := _restore_bindings(data.get("active_bindings", []))
	active_bindings = restored_bindings
	owned_cards = _restore_card_instances(data.get("owned_cards", []))
	deck_cards = _restore_card_instances(data.get("deck_cards", []))
	player_coins = maxi(int(data.get("player_coins", 0)), 0)
	pending_quality_upgrade_percent = maxi(
		int(data.get("pending_quality_upgrade_percent", 0)),
		0
	)
	pending_card_chance_boost_percent = maxi(
		int(data.get("pending_card_chance_boost_percent", 0)),
		0
	)
	pending_guaranteed_card = bool(data.get("pending_guaranteed_card", false))
	pending_force_filled_slots = bool(data.get("pending_force_filled_slots", false))
	_containers.clear()
	_next_card_instance_id = maxi(
		int(data.get("next_card_instance_id", 1)),
		_highest_card_instance_id() + 1
	)
...
func _restore_inventory_slots(value: Variant) -> Array:
	var restored: Array = []
	if value is Array:
		for entry in value:
			if entry is Dictionary:
				var stack: Dictionary = entry.duplicate(true)
				var item_id := int(stack.get("item_id", -1))
				var count := int(stack.get("count", 0))
				if not get_item(item_id).is_empty() and count > 0:
					restored.append({"item_id": item_id, "count": count})
				else:
					restored.append(null)
			else:
				restored.append(null)
	while restored.size() < inventory_capacity:
		restored.append(null)
	if restored.size() > inventory_capacity:
		restored.resize(inventory_capacity)
	return restored


func _restore_bindings(value: Variant) -> Array:
	var restored: Array = []
	if value is Array:
		for item_id in value:
			var parsed_id := int(item_id) if item_id != null else -1
			restored.append(parsed_id if is_active_item(parsed_id) else null)
	while restored.size() < ACTIVE_SLOT_COUNT:
		restored.append(null)
	if restored.size() > ACTIVE_SLOT_COUNT:
		restored.resize(ACTIVE_SLOT_COUNT)
	return restored


func _restore_card_instances(value: Variant) -> Array[Dictionary]:
	...
	for entry in value:
		...
		if get_card(card_id).is_empty() or instance_id < 0:
			continue
		restored.append({
			"instance_id": instance_id,
			"card_id": card_id,
			"remaining_uses": int(entry.get("remaining_uses", get_card_max_uses(card_id))),
		})
	return restored


func _highest_card_instance_id() -> int:
	var highest := 0
	for card_instance in owned_cards:
		highest = maxi(highest, int(card_instance.get("instance_id", 0)))
...
func get_card_max_uses(card_id: int) -> int:
	ensure_initialized()
	var card := get_card(card_id)
	if card.is_empty():
		return -1
	var quality := int(card.get("card_equality", 0))
	return int(card_qualities.get(quality, {}).get("use_num", -1))


func get_deck_card_instances() -> Array[Dictionary]:
	ensure_initialized()
...
func apply_deck_configuration(
	draft_owned_cards: Array[Dictionary],
	draft_deck_cards: Array[Dictionary]
) -> Dictionary:
	ensure_initialized()
	var draft_count := draft_deck_cards.size()
	if draft_count < MIN_DECK_CARD_COUNT:
		return {
			"ok": false,
			"message": "卡组数量不得小于%d张" % MIN_DECK_CARD_COUNT,
		}
	if draft_count > MAX_DECK_CARD_COUNT:
		return {
			"ok": false,
			"message": "卡组数量不得大于%d张" % MAX_DECK_CARD_COUNT,
		}
	var current_ids := _card_instance_ids(owned_cards)
	current_ids.append_array(_card_instance_ids(deck_cards))
	current_ids.sort()
	var draft_ids := _card_instance_ids(draft_owned_cards)
	draft_ids.append_array(_card_instance_ids(draft_deck_cards))
	draft_ids.sort()
	if current_ids != draft_ids:
		return {
			"ok": false,
			"message": "卡牌包与卡组数据不一致，无法保存",
		}
	owned_cards.clear()
	deck_cards.clear()
	for card_instance in draft_owned_cards:
		owned_cards.append(card_instance.duplicate(true))
	for card_instance in draft_deck_cards:
		deck_cards.append(card_instance.duplicate(true))
	_notify_inventory_changed()
	return {"ok": true, "message": "卡组已保存"}


func _card_instance_ids(collection: Array[Dictionary]) -> Array[int]:
	var ids: Array[int] = []
	for card_instance in collection:
		var instance_id := int(card_instance.get("instance_id", -1))
		if instance_id < 0 or ids.has(instance_id):
			return []
		ids.append(instance_id)
	return ids


func consume_card_use(card_instance_id: int) -> int:
	ensure_initialized()
	var remaining := _consume_card_use_in(deck_cards, card_instance_id)
	if remaining != -2:
		return remaining
	remaining = _consume_card_use_in(owned_cards, card_instance_id)
	if remaining != -2:
		return remaining
	return -1


func _consume_card_use_in(
	collection: Array[Dictionary],
	card_instance_id: int
) -> int:
	for index in collection.size():
		var card_instance: Dictionary = collection[index]
		if int(card_instance.get("instance_id", -1)) != card_instance_id:
			continue
		var remaining := int(card_instance.get("remaining_uses", -1))
		if remaining < 0:
			return -1
		remaining = maxi(0, remaining - 1)
		card_instance["remaining_uses"] = remaining
		if remaining == 0:
...
func _create_card_instance(card_id: int) -> Dictionary:
	var instance := {
		"instance_id": _next_card_instance_id,
		"card_id": card_id,
		"remaining_uses": get_card_max_uses(card_id),
	}
	_next_card_instance_id += 1
	return instance
...
func transfer_container_to_inventory(
	...
	if str(stack.get("content_type", "item")) == "card":
		...
		if get_card(card_id).is_empty():
			return "卡牌配置不存在"
		owned_cards.append(_create_card_instance(card_id))
...
func sell_inventory_stacks(inventory_indices: Array[int]) -> Dictionary:
	ensure_initialized()
	var unique_indices: Array[int] = []
	for inventory_index in inventory_indices:
		if (
			inventory_index >= 0
			and inventory_index < inventory_slots.size()
			and not unique_indices.has(inventory_index)
		):
			unique_indices.append(inventory_index)
	unique_indices.sort()

	var sold_stacks := 0
	var sold_items := 0
	var earned := 0
	for inventory_index in unique_indices:
		var stack = inventory_slots[inventory_index]
		if stack == null:
			continue
		var count := int(stack.get("count", 0))
		if count <= 0:
			continue
		earned += get_stack_sell_value(stack)
		sold_items += count
		sold_stacks += 1
		inventory_slots[inventory_index] = null
...
func purchase_shop_card(card_id: int) -> Dictionary:
	ensure_initialized()
	var card := get_card(card_id)
	if card.is_empty() or int(card.get("shop_enabled", 0)) != 1:
		return {"ok": false, "message": "该卡牌未在商店上架"}
	var price := maxi(int(card.get("buy_price", 0)), 0)
	if player_coins < price:
		return {
			"ok": false,
			"message": "金币不足，还差 %d" % (price - player_coins),
		}
	player_coins -= price
	owned_cards.append(_create_card_instance(card_id))
	_notify_inventory_changed()
	return {
		"ok": true,
		"message": "已购买 %s" % str(card.get("card_name", "卡牌")),
		"price": price,
	}
