...
const SHOP_CARD_SIZE := Vector2(88, 125)
...
func _add_shop_card(card_data: Dictionary) -> void:
	...
	cell.custom_minimum_size = Vector2(108, 160)
	cell.alignment = BoxContainer.ALIGNMENT_CENTER
	shop_grid.add_child(cell)
	var center := CenterContainer.new()
	center.custom_minimum_size = Vector2(108, 132)
	cell.add_child(center)
	var card := CARD_VIEW_SCENE.instantiate() as BattleCardView
	center.add_child(card)
	card.configure(card_data, SHOP_CARD_SIZE)
