...
var _focus_scale_factor := 1.0
var _edit_removal_target := false
var _edit_pulse_tween: Tween
var _browse_hover_scale := 1.2
...
func set_hover_active(active: bool) -> void:
	_set_focus_visual(active, _browse_hover_scale)


func set_browse_hover_scale(scale_factor: float) -> void:
	_browse_hover_scale = maxf(scale_factor, 1.0)
	if _focus_visual_active:
		_set_focus_visual(true, _browse_hover_scale)


func set_edit_removal_target(active: bool) -> void:
	_bind_scene_nodes()
	if _edit_removal_target == active:
		return
	_edit_removal_target = active
	if _edit_pulse_tween and _edit_pulse_tween.is_valid():
		_edit_pulse_tween.kill()
		_edit_pulse_tween = null
	if not active:
		_selection_outline.modulate = Color.WHITE
		_refresh_outline_visibility()
		return
	_selection_outline.visible = true
	_selection_outline.modulate = Color(1.0, 0.28, 0.22, 0.92)
	_edit_pulse_tween = create_tween().set_loops()
	_edit_pulse_tween.tween_property(
		_selection_outline,
		"modulate:a",
		0.32,
		0.48
...
func _set_focus_visual(active: bool, scale_factor: float) -> void:
	...
	if (
		...
		and is_equal_approx(_focus_scale_factor, scale_factor)
	):
		_refresh_outline_visibility()
		return
	...
	_focus_visual_active = active
	_focus_scale_factor = scale_factor
	if active:
...
func _finish_invalid_feedback() -> void:
	_selection_outline.modulate = (
		Color(1.0, 0.28, 0.22, _selection_outline.modulate.a)
		if _edit_removal_target
		else Color.WHITE
	)
	_refresh_outline_visibility()


func _refresh_outline_visibility() -> void:
	if not _nodes_bound:
		return
	_selection_outline.visible = (
		_selected_state or _focus_visual_active or _edit_removal_target
	)
