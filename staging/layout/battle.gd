...
const CardPileOverlayScene = preload("res://scenes/ui/battle/card_pile_overlay.tscn")
const CardDetailOverlayScene = preload("res://scenes/ui/card_detail_overlay.tscn")
...
var _player_discard_pile: Array[Dictionary] = []
...
var _sticky_targeting := false
var _is_player_turn := true
var _phase := "setup"
var _battle_over := false
var _player_used_card_instance_ids: Dictionary = {}
...
@onready var _draw_pile_button: Button = %DrawPileButton
@onready var _draw_pile_count: Label = %DrawPileCount
@onready var _discard_pile_button: Button = %DiscardPileButton
@onready var _discard_pile_count: Label = %DiscardPileCount
...
@onready var _card_layout: BattleCardLayout = %CardLayout
...
var _card_pile_overlay: CardPileOverlay
var _card_detail_overlay: CardDetailOverlay


func _ready() -> void:
	_rng.randomize()
	_create_card_pile_overlays()
...
func _input(event: InputEvent) -> void:
	if _dragging_instance_id < 0 or not _sticky_targeting:
		return
	...
	if not _drag_valid:
		return
	var instance_id := _dragging_instance_id
	_resolve_card_release(event.position, instance_id, false)
	get_viewport().set_input_as_handled()
...
func _connect_interface_signals() -> void:
	...
	_draw_pile_button.pressed.connect(_show_draw_pile)
	_discard_pile_button.pressed.connect(_show_discard_pile)


func _create_card_pile_overlays() -> void:
	_card_pile_overlay = CardPileOverlayScene.instantiate() as CardPileOverlay
	add_child(_card_pile_overlay)
	_card_pile_overlay.card_detail_requested.connect(_show_card_detail)
	_card_detail_overlay = CardDetailOverlayScene.instantiate() as CardDetailOverlay
	add_child(_card_detail_overlay)


func _show_draw_pile() -> void:
	_card_pile_overlay.show_pile("抽牌堆", _player_deck, _cards)


func _show_discard_pile() -> void:
	_card_pile_overlay.show_pile("弃牌堆", _player_discard_pile, _cards)
...
func _show_card_detail(
	...
	_card_detail_overlay.show_card(card_data, runtime_state)
...
func _draw_card(for_player: bool, announce := true) -> bool:
	...
	if hand.size() >= HAND_LIMIT:
		...
		return false
	if for_player and _player_deck.is_empty():
		_refill_player_draw_pile(announce)
	if (for_player and _player_deck.is_empty()) or (
		not for_player and _enemy_deck.is_empty()
	):
		if announce:
...
func _can_draw_player_card() -> bool:
	return not _player_deck.is_empty() or not _player_discard_pile.is_empty()


func _refill_player_draw_pile(announce: bool) -> bool:
	if not _player_deck.is_empty() or _player_discard_pile.is_empty():
		return false
	_player_deck = _player_discard_pile.duplicate(true)
	_player_discard_pile.clear()
	_player_deck.shuffle()
	if announce:
		_write_log("抽牌堆已空，弃牌堆洗回抽牌堆。")
	return true


func _on_hand_card_pressed(
	instance_id: int,
	...
	if not _can_start_card_interaction(instance_id):
		source_view.play_invalid_feedback()
		_event_label.text = _card_unavailable_reason(instance_id)
		return
	if _sticky_targeting and instance_id == _dragging_instance_id:
		return
	if _dragging_instance_id >= 0:
		_target_overlay.cancel_immediately()
		_reset_drag_state(true)
		_selected_instance_id = -1
	_begin_card_targeting(
...
func _begin_card_targeting(
	...
	_event_label.text = (
		"已选择 %s：左键有效目标释放，右键取消" % card["card_name"]
		if sticky
		else "拖动 %s 选择目标" % card["card_name"]
	)
...
func _on_hand_card_drag_ended(
	...
	if instance_id != _dragging_instance_id:
		return
	_resolve_card_release(screen_position, instance_id, true)


func _resolve_card_release(
	screen_position: Vector2,
	instance_id: int,
	cancel_when_invalid: bool
) -> void:
	_update_card_drag(screen_position)
	var hand_index := _find_hand_index(_player_hand, instance_id)
	if hand_index < 0:
...
func _card_unavailable_reason(instance_id: int) -> String:
	if _battle_over:
		return "战斗已经结束"
	if not _is_player_turn:
		return "当前不是玩家回合"
	if _phase != "player_input":
		return "当前阶段不能使用卡牌"
	var hand_index := _find_hand_index(_player_hand, instance_id)
	if hand_index < 0:
		return "这张卡牌已不在手牌中"
	...
	if int(card.get("cost", 0)) > _player_energy:
		return "能量不足，无法使用 %s" % card["card_name"]
	return "当前无法使用这张卡牌"
...
func _play_function_card(
	...
	_player_energy -= cost
	_mark_player_card_used(hand_card)
	_player_hand.remove_at(hand_index)
	_player_discard_pile.append(_to_persistent_card(hand_card))
	if card_id == 9:
		_draw_card(true, false)
	_selected_instance_id = -1
...
func _play_player_card(slot_index: int) -> void:
	...
	_player_board[slot_index]["remaining_uses"] = int(
		hand_card.get("remaining_uses", -1)
	)
...
func _mark_player_card_used(hand_card: Dictionary) -> void:
	var persistent_instance_id := int(
		hand_card.get("persistent_instance_id", -1)
	)
	if persistent_instance_id >= 0:
		_player_used_card_instance_ids[persistent_instance_id] = true


func _to_persistent_card(hand_card: Dictionary) -> Dictionary:
	return {
		"instance_id": int(hand_card.get("persistent_instance_id", -1)),
		"card_id": int(hand_card.get("card_id", -1)),
		"remaining_uses": int(hand_card.get("remaining_uses", -1)),
	}


func _settle_player_card_uses() -> void:
	for persistent_instance_id in _player_used_card_instance_ids:
		InventoryState.consume_card_use(int(persistent_instance_id))
	_player_used_card_instance_ids.clear()
...
func _refresh_all() -> void:
	...
	_draw_pile_count.text = "%d" % _player_deck.size()
	_discard_pile_count.text = "%d" % _player_discard_pile.size()
	_deck_label.text = "我方手牌 %d    敌方牌库 %d / 手牌 %d" % [
		_player_hand.size(),
		_enemy_deck.size(),
		_enemy_hand.size(),
	]
...
func _render_slot(
	...
	card_view.configure(_cards[card_id], _card_layout.slot_card_size, hero)
	card_view.position = _card_layout.slot_card_offset
...
func _refresh_hand() -> void:
	...
	for hand_card in _player_hand:
		...
		card_cell.custom_minimum_size = (
			_card_layout.card_size * _card_layout.hover_scale
			+ _card_layout.cell_padding
		)
		card_cell.mouse_filter = Control.MOUSE_FILTER_PASS
		_hand_container.add_child(card_cell)
		var card_view = CardViewScene.instantiate()
		card_cell.add_child(card_view)
		card_view.configure(card, _card_layout.card_size, hand_card)
		...
		card_view.card_clicked.connect(
			_on_hand_card_pressed.bind(instance_id, card_view)
		)
