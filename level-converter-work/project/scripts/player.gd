...
var _world_map_size := WORLD_MAP_SIZE
...
func _update_camera_for_viewport() -> void:
	...
	var map_fit_zoom := maxf(
		viewport_size.x / _world_map_size.x,
		viewport_size.y / _world_map_size.y
	)
...
func configure_world_bounds(map_rect: Rect2) -> void:
	_world_map_size = map_rect.size
	if camera:
